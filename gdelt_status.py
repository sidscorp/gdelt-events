...
from pipeline.config import DATA_DIR, DB_PATH, RAW_DIR
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"  GDELT v2 Pipeline Status")
    print(f"{'='*55}")
    ...
    for table, ts_col in [("events", "DATEADDED"), ("mentions", "MentionTimeDate"), ("gkg", "V1DATE")]:
        try:
            row = con.execute(
