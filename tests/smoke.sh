...
check "views has devices"         "/api/views"                                                             "any(v['id'] == 'medical-devices' for v in d['views'])"
check "views default_hours"       "/api/views"                                                             "next(v for v in d['views'] if v['id']=='medical-devices').get('default_hours') == 24"
...
check "gal devices view 3d"       "/api/articles?source=gal&view=medical-devices&hours=72"                 "isinstance(d.get('articles'), list)"
...
check "gkg devices view 3d"       "/api/articles?source=gkg&view=medical-devices&hours=72"                 "isinstance(d.get('articles'), list)"

echo ""
echo "=== source=all ==="
check "all 1h"                    "/api/articles?source=all&hours=1"                                       "isinstance(d.get('articles'), list) and isinstance(d.get('total'), int)"
check "all devices view 3d"       "/api/articles?source=all&view=medical-devices&hours=72"                 "d['total'] > 0"
